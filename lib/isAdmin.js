...
async function isAdmin(sock, chatId, senderId) {
    try {
        ...
        const ownerNumbers = [settings.ownerNumber]
            .filter(Boolean)
            .map(number => number.toString().replace(/[^0-9]/g, ''));
        ...
        const botNumbers = [
            ...
            .filter(Boolean)
            .map(value => value.toString().replace(/[^0-9]/g, ''))
            .filter(Boolean);

        const isPrivilegedSender = ownerNumbers.includes(senderNumber.replace(/[^0-9]/g, '')) ||
            botNumbers.includes(senderNumber.replace(/[^0-9]/g, '')) ||
            botNumbers.includes(senderIdWithoutSuffix.replace(/[^0-9]/g, ''));
        ...
        const isSenderAdmin = isPrivilegedSender || participants.some(p => {
            // Check multiple possible ID formats
            const pPhoneNumber = p.phoneNumber ? p.phoneNumber.split('@')[0] : '';
            const pId = p.id ? p.id.split('@')[0] : '';
            const pLid = p.lid ? p.lid.split('@')[0] : '';
