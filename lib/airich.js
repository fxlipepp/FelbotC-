        ...
        if (type === 'latex') {
          ...
          try {
            ...
            if (hashIdx !== -1) {
              ...
              if (params.has('font_height')) font_height = Number(params.get('font_height'));
              if (params.has('padding')) padding = Number(params.get('padding'));
...
class Toolkit {
  ...
  static async resolveLatexDimensions(url, font_height) {
    ...
    const estHeight = formula.includes('\\frac') || formula.includes('\\sum') || formula.includes('\\int') ? 32 : 20;
  ...
  static async resolveMedia(_client, media, mediaType = 'image', { resolveUrl = false, resolveWAUrl = false, result = 'url', resize = false, width = 300, height = 300 } = {}) {
    ...
    if (Array.isArray(media)) {
      return Promise.all(
        media.map((item) => Toolkit.resolveMedia(_client, item, mediaType, { resolveUrl, resolveWAUrl, result, resize, width, height })),
      );
    }
  ...
  static getMp4Preview(videoBuffer, { time, result = 'buffer', resize = true, width = 300, height = 300, silent = true } = {}) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        if (!ffmpeg) return fail(new Error('fluent-ffmpeg is not installed'));
...
class BaseBuilder {
  ...
  static async resize(buffer, x, y, fit = 'cover') {
    return Toolkit.resize(buffer, x, y, fit);
  }

  static async fetchBuffer(url, options = {}, config = {}) {
    return Toolkit.fetchBuffer(url, options, config);
  }
}
...
class Button extends BaseBuilder {
  ...
  async setLocation(thumbnail, name = '', address = '') {
    ...
    if (thumbnail) {
      const buf = Buffer.isBuffer(thumbnail) ? thumbnail : await BaseBuilder.fetchBuffer(thumbnail, {}, { silent: true });
      if (buf.length > 0) {
...
class ButtonV2 extends BaseBuilder {
  ...
  setVideo(videoPath) {
    ...
    this._videoBuffer = Buffer.isBuffer(videoPath) ? videoPath : null;
    return this;
  }
  ...
  async build(jid, { mentions, ...options } = {}) {
    ...
    if (this._videoPath) {
      const videoSource = this._videoBuffer
        ? this._videoBuffer
        : (typeof this._videoPath === 'string'
            ? this._videoPath
            : this._videoPath?.url || this._videoPath?.path);

      if (videoSource) {
        const resolvedVideoPath = Buffer.isBuffer(videoSource) ? null : path.resolve(String(videoSource));
        const fileBuffer = Buffer.isBuffer(videoSource)
          ? videoSource
          : (resolvedVideoPath && fs.existsSync(resolvedVideoPath) ? fs.readFileSync(resolvedVideoPath) : null);
          ...
          try {
            ...
            headerPayload = {
              ...
              videoMessage: {
                ...
                gifPlayback: true,
                mimetype: prepared?.videoMessage?.mimetype || 'video/mp4',
                caption: this._body,
              },
            };
          } catch (error) {
            headerPayload = {
              headerType: 1,
              textMessage: { text: this._body },
            };
          }
        }
      }
    } else if (this._data) {
      headerPayload = this._data;
    }
  ...
  async send(jid, { ...options } = {}) {
    ...
    try {
      if (this.#client?.sendMessage && this._videoPath) {
        const videoSource = this._videoBuffer
          ? this._videoBuffer
          : (typeof this._videoPath === 'string'
              ? this._videoPath
        ...
        if (videoSource) {
          const payload = {
            video: Buffer.isBuffer(videoSource) ? videoSource : { url: String(videoSource) },
            caption: this._body,
            footer: this._footer,
            gifPlayback: true,
            mimetype: 'video/mp4',
            buttons: this._buttons.map((button) => ({
              buttonId: button.buttonId,
              buttonText: button.buttonText || { displayText: button.displayText || '' },
              type: button.type || 1,
            })),
          };

          return this.#client.sendMessage(jid, payload, { ...options });
        }
      }
    } catch (error) {
      console.error('ButtonV2 video send fallback error:', error);
    }
...
class AIRich extends BaseBuilder {
  ...
  addText(text, { hyperlink = true, citation = true, latex = true } = {}) {
    ...
    if (this._sections.length > 0) {
      ...
      if (lastType && lastType !== 'GenAIMarkdownTextUXPrimitive' && lastText !== ' ') {
    ...
    this._sections.push(AIRich.newLayout('Single', {
      ...
      ...(extractedIE.inline_entities.length ? { inline_entities: extractedIE.inline_entities } : {}),
  ...
  addSource(sources = []) {
    ...
    const normalized = sources.every((item) => typeof item === 'string') ? [sources] : sources;
  ...
  addReels(reelsItems = []) {
    ...
    const reels = (Array.isArray(reelsItems) ? reelsItems : [reelsItems]).map((item) => ({
      ...item,
