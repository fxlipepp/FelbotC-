...
async function textmakerCommand(sock, chatId, message, q, type) {
    try {
        if (!q) {
            return await sock.sendMessage(chatId, messageTemplates.error("Por favor proporciona un texto\nEjemplo: .metallic Nick"));
        }
        ...
        try {
            ...
            switch (type) {
                ...
                case 'fire':
                    ...
                    break;
                default:
                    return await sock.sendMessage(chatId, messageTemplates.error("Tipo de generador inválido"));
            }
            ...
            console.error('Error en el generador de texto:', error);
            await sock.sendMessage(chatId, messageTemplates.error(`Error: ${error.message}`));
        }
    } catch (error) {
        console.error('Error en el comando textmaker:', error);
        await sock.sendMessage(chatId, messageTemplates.error("Ocurrió un error. Inténtalo más tarde."));
    }
}
